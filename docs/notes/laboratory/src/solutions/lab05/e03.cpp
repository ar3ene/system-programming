...
Computed as: v[0]=v[0], v[1]=v[1], v[2]=v[2], v[3]=v[3],
...
using namespace std;
 ...
 void thread_func(vector<int> &v, int n, int dim, Barrier &barrier){
    int sum;
    for (int i=0; i<dim ; ++i)
    {
        int p = n- (1 << i);
...
int main(int argc, char *argv[]) {
    ...
    std::uniform_int_distribution<unsigned int> dist(1, 9);
    ...
    vector<thread> vt(pnum-1);

    Barrier bar(pnum-1);

    for (int i=0; i<pnum-1; ++i) {
        vt[i] = thread(thread_func, ref(vet), i+1, num, ref(bar));
    }
