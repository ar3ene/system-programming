...
void thread_func(double *a, double **b, double *c, double *t, int k, int n, mutex &mtx, int &finished, double &r) {
    double res = 0;
    for (int i=0; i<n; i++)
        t[k]+=a[i]*b[i][k];
    {
        lock_guard<mutex> lock(mtx);
        //cout << "Task " << k << " finished" << endl;
        finished++;
    }
    if (finished == n) {
        cout << "Task " << k << " computing result" << endl;
        for (int i = 0; i < n; i++) {
            r += t[i] * c[i];
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    for (int i=0; i<num; ++i) {
        vet_thr.emplace_back(thread(thread_func, v1, mat, v2, vtemp, i, num, ref(mtx), ref(finished), ref(res)));
    }
